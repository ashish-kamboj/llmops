...
def load_users() -> List[dict]:
    resp = requests.get(f"{API_BASE_URL}/users", timeout=10)
    resp.raise_for_status()
    return resp.json()


def get_recommendations(page_id: str, user_id: str | None = None) -> List[dict]:
    params = {}
    if user_id:
        params["user_id"] = user_id
    resp = requests.get(f"{API_BASE_URL}/recommend/{page_id}", params=params, timeout=10)
    resp.raise_for_status()
    return resp.json()["results"]


def log_click(user_id: str, page_id: str) -> None:
    try:
        requests.post(
            f"{API_BASE_URL}/click",
            params={"user_id": user_id, "page_id": page_id, "event_type": "view"},
            timeout=5,
        )
    except Exception:
        pass
...
def main() -> None:
    ...
    users = load_users()
    user_labels = {f"{u['name']} ({u['team']})": u["id"] for u in users}
    user_labels["Anonymous (no personalization)"] = None

    with st.sidebar:
        st.markdown("<div class='sidebar-title'>Logged in as</div>", unsafe_allow_html=True)
        ...
        current_user_id = user_labels[selected_user_label]

        st.markdown("---", unsafe_allow_html=True)
        st.markdown("<div class='sidebar-title'>Pages</div>", unsafe_allow_html=True)
    ...
    # Log this page view
    if current_user_id:
        log_click(current_user_id, page_id)
    ...
    recs = get_recommendations(page_id, user_id=current_user_id)

    cols = st.columns(3)
    for idx, rec in enumerate(recs):
        with cols[idx % 3]:
            score_parts = f"Score: {rec['similarity']:.3f}"
            if current_user_id:
                score_parts += (
                    f" (semantic {rec.get('semantic_score', 0):.2f}"
                    f" / click {rec.get('click_score', 0):.2f}"
                    f" / recency {rec.get('recency_score', 0):.2f})"
                )
            st.markdown(
                f"""
                <div class="card">
                    <div class="card-title">{rec['title']}</div>
                    <div class="card-sub">{score_parts}</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
