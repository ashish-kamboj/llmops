...
- Serves page content and top-6 related page recommendations through FastAPI.
- Renders a Confluence-like UI in Streamlit with left navigation and related content cards.
...
3. Embedding + load:
   - `src/pipelines/embed_pages.py` calls Gemini embedding model (`models/gemini-embedding-001`), then upserts into:
     ...
     - `page_embeddings`
4. API retrieval:
   - `src/app/main.py` exposes `/pages`, `/pages/{page_id}`, `/recommend/{page_id}`.
   - Recommendation query computes cosine distance in Postgres vector column and returns top-K (default 6).
5. UI:
   - `ui/streamlit_app.py` fetches API data and shows selected page + related content.
...
```text
      ...
      | /pages /recommend      |<--| Confluence-like page   |
      +------------------------+   +------------------------+
```
...
- [src/app](src/app): FastAPI service
- [src/pipelines](src/pipelines): data generation + extraction + embedding
